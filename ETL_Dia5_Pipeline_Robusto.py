    ...
    def transform_with_validation(self, data):
        ...
        try:
            ...
            if data.isnull().any().any():
                ...
                self.logger.warning(
                    f"Valores nulos encontrados: {null_counts[null_counts > 0].to_dict()}"
                )
            ...
            self.metrics['processed'] = len(data_clean)
    ...
    def load_with_transaction(self, data):
        ...
        with sqlite3.connect(self.db_path) as conn:
            try:
                ...
                self.logger.error(
                    f"Error en carga, ejecutando rollback: {e}"
                )
                raise
